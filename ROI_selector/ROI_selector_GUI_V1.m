...
if (handles.compNb == 1) %Added this in case last batch of ROIs to select is only of size 1 to avoid slider error
    handles.compNb = 2;
end
...
plot(handles.vars.Coor_kp{initComp}(1,:),handles.vars.Coor_kp{initComp}(2,:),'r', 'LineWidth',1);
    ...
    fprintf(e.message)
    warning('Remove this corrupted ROI')
end
hold off
...
next_ROI_Callback(handles.next_ROI, eventdata, handles);
