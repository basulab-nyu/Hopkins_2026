function run_pipeline_CNMF_only_nr(iterNum)
...
%navigate to experiment directory - need this otherwise with write output
%to wrong folder
cd(foldername);
...
%    hold on
%    ax1 = subplot(121); 
Coor_kp = plot_contours(A(:,keep),Cn,options,0,[],Coor_k,[],1,find(keep)); title('Selected components','fontweight','bold','fontsize',14);
%    %scatter(newcenters(:,2),newcenters(:,1),'yo');
%    hold off
%    ax2 = subplot(122); 
%plot_contours(A(:,throw),Cn,options,0,[],Coor_t,[],1,find(throw));title('Rejected components','fontweight','bold','fontsize',14);
%    linkaxes([ax1,ax2],'xy')
