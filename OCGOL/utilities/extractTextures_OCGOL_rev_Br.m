function [Behavior] = extractTextures_OCGOL_rev_Br(CSV, Behavior,options)
...
[trialRanges] = discoverTrialType_oneTrial(trialTypeCh, position);

%get time and position of trial textures (may change to findpeaks approach
%like done above depending on stability)
[trialType] = defineTrialSignal_oneTrial(trialRanges, trialTypeCh, Behavior);
...
if strcmpi(options.BehaviorType,'OCGOL') || strcmpi(options.BehaviorType,'OCGOL-rev') || strcmpi(options.BehaviorType,'OCGOL-rev-Br')
