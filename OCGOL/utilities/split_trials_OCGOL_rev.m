function [Behavior_split,Imaging_split,Events_split,Behavior_split_lap,Events_split_lap] = split_trials_OCGOL_rev(Behavior, Imaging, Events, options)
...
% %plot whole A trace as continuous frames %MH commented out 3/3/25 for
% % Br only trial days - this can typically be uncommented
% %plot whole A trace as continuous frames
% plot(traces_split{4}(:,ROI),'k')
% 
% %shade traces that are in run epochs green
% for ii=1:size(run_on_off_idx{4},1)
%     plot(run_on_off_idx{4}(ii,1):run_on_off_idx{4}(ii,2),...
%         traces_split{4}(run_on_off_idx{4}(ii,1):run_on_off_idx{4}(ii,2),ROI),'Color', [0 100 0]/255);
%     %shades norm positions below
%     plot(run_on_off_idx{4}(ii,1):run_on_off_idx{4}(ii,2),...,
%         positions_norm_split{4}(run_on_off_idx{4}(ii,1):run_on_off_idx{4}(ii,2))-1.2,'Color', [0 100 0]/255)
%     %mark the significant events on the lap position
%     stem(find(Events_split{4}.Run.run_onset_binary(:,ROI) ==1),...
%         positions_norm_split{4}(logical(Events_split{4}.Run.run_onset_binary(:,ROI)))-1.2, 'LineStyle','none',...
%         'MarkerFaceColor',[0 100 0]/255,'MarkerEdgeColor',[0 100 0]/255)
% end
% 
% %shade traces that are in not in run epochs red
% for ii=1:size(norun_on_off_idx{4},1)
%     plot(norun_on_off_idx{4}(ii,1):norun_on_off_idx{4}(ii,2),...
%         traces_split{4}(norun_on_off_idx{4}(ii,1):norun_on_off_idx{4}(ii,2),ROI),'r');
%     %shades norm positions below
%     plot(norun_on_off_idx{4}(ii,1):norun_on_off_idx{4}(ii,2),...,
%         positions_norm_split{4}(norun_on_off_idx{4}(ii,1):norun_on_off_idx{4}(ii,2))-1.2,'Color', 'r')
%         %mark the significant events on the lap position
%     stem(find(Events_split{4}.NoRun.norun_onset_binary(:,ROI) ==1),...
%         positions_norm_split{4}(logical(Events_split{4}.NoRun.norun_onset_binary(:,ROI)))-1.2, 'LineStyle','none',...
...
hold on
title('B trials')
plot(traces_split{5}(:,ROI),'k')

%shade traces that are in run epochs green
for ii=1:size(run_on_off_idx{5},1)
    plot(run_on_off_idx{5}(ii,1):run_on_off_idx{5}(ii,2),...
        traces_split{5}(run_on_off_idx{5}(ii,1):run_on_off_idx{5}(ii,2),ROI),'Color', [0 100 0]/255);
        %shades norm positions below
    plot(run_on_off_idx{5}(ii,1):run_on_off_idx{5}(ii,2),...,
        positions_norm_split{5}(run_on_off_idx{5}(ii,1):run_on_off_idx{5}(ii,2))-1.2,'Color', [0 100 0]/255)
    %mark the significant events on the lap position 
    stem(find(Events_split{5}.Run.run_onset_binary(:,ROI) ==1),...
        positions_norm_split{5}(logical(Events_split{5}.Run.run_onset_binary(:,ROI)))-1.2, 'LineStyle','none',...
        'MarkerFaceColor',[0 100 0]/255,'MarkerEdgeColor',[0 100 0]/255)

end
%shade traces that are in not in run epochs red
for ii=1:size(norun_on_off_idx{5},1)
    plot(norun_on_off_idx{5}(ii,1):norun_on_off_idx{5}(ii,2),...
        traces_split{5}(norun_on_off_idx{5}(ii,1):norun_on_off_idx{5}(ii,2),ROI),'r');
    %plot position
        plot(norun_on_off_idx{5}(ii,1):norun_on_off_idx{5}(ii,2),...,
        positions_norm_split{5}(norun_on_off_idx{5}(ii,1):norun_on_off_idx{5}(ii,2))-1.2,'Color', 'r')
    %mark the significant events on the lap position
            %mark the significant events on the lap position
    stem(find(Events_split{5}.NoRun.norun_onset_binary(:,ROI) ==1),...
        positions_norm_split{5}(logical(Events_split{5}.NoRun.norun_onset_binary(:,ROI)))-1.2, 'LineStyle','none',...
        'MarkerFaceColor','r','MarkerEdgeColor','r')
end
