...
for cc=1:SCE{ss}.nbSCE
    [sorted_onset_times{cc},sce_sort_order{cc}] = sort(max_onset{cc},'ascend');
    %re-sort the SCE-associated ROIs based on this order
     ...
     %sort SCE traces
     SCE_traces_sorted{cc} = SCE_traces{cc}(:,sce_sort_order{cc});
end

%% Plot time sorted transients
figure;
%first 5 for show
...
stepSize = 2;
step = 0;   
    for ii=1:size(SCE_unique_ROIs_noise_rm{cc},2)
        plot(SCE_traces_sorted{cc}(:,ii), 'LineWidth', 1.5)
        %plot(dur_filtered_event(:,ii)-step, 'r', 'LineWidth', 1.5)
        step = step - stepSize;
        %pause;
    end
...
SCE{ss}.sorted_onset_times = sorted_onset_times;
