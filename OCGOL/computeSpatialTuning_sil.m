function dataStruct = computeSpatialTuning_sil(matchedMice, ca_files, cent_files, trialType, day)
...
% day: which session number from number of sessions in cross_update files
...
dataStruct.si_scores = cell(1, nMice); %filtered for SI tuned cells
dataStruct.ts_scores = cell(1, nMice); %filtered for SI tuned cells
dataStruct.si_scores_all = cell(1, nMice); %SI for all cells (incl untuned)
dataStruct.ts_scores_all = cell(1, nMice); %TS for all cells (incl untuned)
...
for i = 1:nMice
        ...
        dataStruct.si_scores_all{i} = all_si_scores;
        dataStruct.ts_scores_all{i} = all_ts_scores;
        ...
        if length(loaded_2.cent_diff.all_sig_bin)<4 %This should only affect MH115 which is missing saline d1
            centMatrix=loaded_2.cent_diff.all_sig_bin{1,day-1}{1,trialType};
            centMatrix_si= centMatrix(si_idx);
            centMatrix_ts= centMatrix(ts_idx);
        elseif length(loaded_2.cent_diff.all_sig_bin)>=4
            centMatrix=loaded_2.cent_diff.all_sig_bin{1,day}{1,trialType};
            centMatrix_si= centMatrix(si_idx);
            centMatrix_ts= centMatrix(ts_idx);
        end
