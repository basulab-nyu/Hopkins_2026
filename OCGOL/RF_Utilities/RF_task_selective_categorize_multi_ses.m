...
for ss=sessionSelect
    ...
    Aonly_notSIb_idx{ss} = find(tuned_logical(ss).ts.Atuned == 1);
    
    %only A tuned by SI, but also not TS B tuned
    Aonly_notTSb_idx{ss} = find(tuned_logical(ss).si.Atuned == 1);
...
for ss=sessionSelect
    ...
    for tt=selectTrial
        ...
        for rr=1:size(lap_idx_range{ss}{tt},2)
            ...
            if tt == selectTrial(1) %if looking on run status in B trials for correct A trials
                lap_opposed_idx{ss}{tt}{rr} = ismember(lap_idx_range{ss}{tt}{rr},corr_lap_idx{ss}{selectTrial(1)}); %TODO MH changed selectTrial(2)-->(1) as workaround
            end
    ...
    for tt=selectTrial
        ...
        for rr=1:size(lap_idx_range{ss}{tt},2)
            ...
            if tt == selectTrial(1) %if looking on run status in B trials for correct A trials
                for ll=1:size(corr_lap_idx{ss}{selectTrial(1)},1) %TODO MH changed selectTrial(2)-->(1) as workaround (number of "A" trial laps")
                    % - depdendent on B parameter
                    split_lap_idxs{ss}{tt}{rr}{ll} = find(lap_label_opposed{ss}{tt}{rr} == corr_lap_idx{ss}{selectTrial(1)}(ll)); %TODO MH changed selectTrial(2)-->(1) as workaround                    
