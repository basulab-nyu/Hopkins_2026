...
for ss=sessionSelect
    ...
    %SI
    tuned_count_filt{ss}{1}(1) = size(find((tunedLogical(ss).si.Atuned & pf_count_filtered_log{ss}(selectTrial(1),:)) ==1),2);
    tuned_count_filt{ss}{1}(4) = size(pf_count_filtered_log{ss},2) -  sum(tuned_count_filt{ss}{1}(1));
    
    %TS
    tuned_count_filt{ss}{2}(1) = size(find((tunedLogical(ss).ts.Atuned & pf_count_filtered_log{ss}(selectTrial(1),:)) ==1),2);
    tuned_count_filt{ss}{2}(4) = size(pf_count_filtered_log{ss},2) -  sum(tuned_count_filt{ss}{2}(1));
...
for ss=sessionSelect
    ...
    tuned_log_filt_si{ss}.neither = ~(tuned_log_filt_si{ss}.allA);
    ...
    tuned_log_filt_ts{ss}.neither = ~(tuned_log_filt_ts{ss}.allA);
...
%% Organize absolute numbers for each category
%place into struct for SI and TS tuned (row = session; column =
%A,B,A&B,Neither
for ss=sessionSelect
    %1 is SI
    tuned_counts_ordered.si(ss,:) = tuned_count_filt{ss}{1};
    %2 is TS
    tuned_counts_ordered.ts(ss,:) = tuned_count_filt{ss}{2};
end

%QC check to see if I get the same fractions
% si_frac_check = tuned_counts_ordered.si./sum(tuned_counts_ordered.si,2);
% ts_frac_check = tuned_counts_ordered.ts./sum(tuned_counts_ordered.ts,2);
% %confirms
% isequal(si_frac_check, fracTuned_si_filt)
% isequal(ts_frac_check, fracTuned_ts_filt)
...
%count ordered in matrix form for session and subtype
tuned_fractions.tuned_counts_ordered = tuned_counts_ordered;
