...
for ss=1:size(match_list,2)
     A_less_than_5_events = find(session_vars{ss}.Events_split{1}.Run.properties.nb_events < min_run_events);
     
     %intersect the 2 sets of ROIs
     neuron_idx_less_than_5_AandB{ss} = A_less_than_5_events; 
end
...
for ss=1:size(session_vars,2)
    ...
    event_map{ss}.A = session_vars{ss}.Place_cell{1}.Spatial_Info.rate_map{8}';
    %occupancy in seconds
    occupancy_s{ss}.A = session_vars{ss}.Place_cell{1}.Spatial_Info.occupancy_map{8};
...
for ss=1:size(session_vars,2)
    STC_norm{ss}.A = session_vars{ss}.Place_cell{1}.Spatial_tuning_curve;
end

%non-normalized
for ss=1:size(session_vars,2)
    STC_nonNorm{ss}.A = session_vars{ss}.Place_cell{1}.Spatial_tuning_curve_no_norm;
end
...
[session_STC_norm_min_5_events_global] = RF_multi_session_STC_extract(match_list_run_event_filtered_5_event_global,STC_norm);

%% Extract matched non-normalized STCs - at least 5 global events

[session_STC_nonNorm_min_5_events_global] = RF_multi_session_STC_extract(match_list_run_event_filtered_5_event_global,STC_nonNorm);

%% Extract matched smoothed rate maps - at least 5 global events (smoothed events over smoothed occupancy)

[session_sm_rate_map_min_5_events_global] = RF_multi_session_STC_extract(match_list_run_event_filtered_5_event_global,rate_map_ev_sm_oc_sm);

%% Extract matched normalized STCs - at least 5 run event on A and B trials

[session_STC_norm_min_5_events_both_A_B] = RF_multi_session_STC_extract(match_list_run_event_filtered_5_event_both_A_B,STC_norm);

%% Extract matched non-normalized STCS - at least 5 run events on A and B trials

[session_STC_nonNorm_min_5_events_both_A_B] = RF_multi_session_STC_extract(match_list_run_event_filtered_5_event_both_A_B,STC_nonNorm);

%% Extract matched smoothed rate maps - at least 5 run events on A and B trials (smoothed events over smoothed occupancy)

[session_sm_rate_map_min_5_events_both_A_B] = RF_multi_session_STC_extract(match_list_run_event_filtered_5_event_both_A_B,rate_map_ev_sm_oc_sm);
