function result = run_PosDecoding_vis(matFilePath, ses, indiceValue, perfValue, color, downSamp)
...
%     ses: identifies which session of cross registered sessions to plot
%     indiceValue: trial type to use (2 or 3) - RF:3, OCGOLA:2, OCGOLB:3
%     perfValue: OCGOL=1,RF=0
%     color: RGB code for plots
...
%     result: struct with performance and error metrics
    ...
    if doDownSamp
        rois=size(these{1}.transient_mat,1); %total number of rois in this condition
        if downSamp > rois
    ...
    figure('Position',[1 1 650 165]);
        ...
        subplot(1,2,1);
        pred_time_scaled = t2(pp_indice);
        plot(pred_time_scaled-pred_time_scaled(1), dp2(pp_indice), '.','Color',[0.5 0.5 0.5]);
        hold on;
        plot(pred_time_scaled-pred_time_scaled(1), pp2(pp_indice), 'r.', 'Color', color);
        ...
        plot(dp2+randn(size(dp2)),pp2+randn(size(dp2)),'o','Color',color,'Markersize',3);
        c = corrcoef(dp2,pp2,'rows','complete');
        title(sprintf('R = %.02f', c(1,2)));
