function dataStruct = processCaDynamics_ldur(matchedMice,ca_files,trialType,sampleTime)
...
%sampleTime: size of time window for average event trace in 30 Hz units
%all events will be greater than 2 seconds longs
...
for i = 1:nMice
        ...
        for roi = 1:numel(durations)
            if ~isempty(durations{roi})
                % Create binary vector: 1 if duration >2 seconds (hard coded), else 0
                durationThreshold= 2; % seconds
                binarizedDur{roi} = durations{roi} > durationThreshold; 
            else
        ...
        %calculate average event trace for all long duration events------
        ...
        for roi = 1:numel(dataStruct.CaDynamic.dur_binarized{i})
            ...
            binarized = dataStruct.CaDynamic.dur_binarized{i}{roi};  % Logical vector of >2s events for a given ROI
            traces = events.Run.properties.trace{roi};            % Cell array of event traces of all durations for a given roi
            ...
            for evt = 1:length(traces)
                if binarized(evt)  % Only process events > 2 seconds
                    ...
                    elseif len > sampleTime
                        trace = trace(1:sampleTime);
                    else
                    end
        ...
        for roi = 1:numel(dataStruct.CaDynamic.dur_binarized{i})
            ...
            binarized = dataStruct.CaDynamic.dur_binarized{i}{roi};  % Logical vector of >2s events for given roi
            ...
            AUC_roi = AUCs(binarized)';  % Get the AUC for >2s events
        ...
        for roi = 1:numel(dataStruct.CaDynamic.dur_binarized{i})
            ...
            amp_roi = amps(binarized)';  % Get the amplitude for long events
