function [theta, unit_vectors, circular_mean_rad, circular_mean_cm] = centroid_shift_taskDemand(pathName,type)
    ...
    for tt = options.selectTrial
        ...
        if size(ROI_assign_multi_filtered, 2) == 4 %Handles subsampled sessions and gets ROIs that are matched only between subsampled sessions
            switch type
                case 'RF'
                    options.ROIfilt_sess = [1 2];
                case 'OCGOL'
                    options.ROIfilt_sess = [3 4];
                otherwise
                    error('Unknown type: %s. Must be ''RF'' or ''OCGOL''.', type);
            end
            fprintf('\nFiltering matched ROIs for trial %d.\n',tt);
            validROIMatch = ~isnan(ROI_assign_multi_filtered(:,options.ROIfilt_sess(1))) & ~isnan(ROI_assign_multi_filtered(:,options.ROIfilt_sess(2)));
            validROI_filtered=ROI_assign_multi_filtered(validROIMatch,options.ROIfilt_sess);

            tuned_both_idx = validROI_filtered( ...
            tuned_s1(validROI_filtered(:,1)) & ...
            tuned_s2(validROI_filtered(:,2)), :);
        else
            tuned_both_idx = ROI_assign_multi_filtered( ...
            tuned_s1(ROI_assign_multi_filtered(:,1)) & ...
            tuned_s2(ROI_assign_multi_filtered(:,2)), :);
        end
    ...
    for trial=options.selectTrial
        ...
        switch type
            case 'RF'
                    ...
                    for ii=1:nPlot
                        subplot(5,10,ii)
                        compass(pf_vector_max{options.sessionSelect(1)}{trial-3}(tuned_both_idx_plot{trial}(ii,1)),'r')
                        hold on;
                        compass(pf_vector_max{options.sessionSelect(2)}{trial-3}(tuned_both_idx_plot{trial}(ii,2)),'r')
        ...
        %Check that ther are no NaNs in theta %
        nanCount = sum(isnan(theta{trial}));
        if nanCount > 0
            fprintf('Warning: Trial %d contains %d NaNs. Removing them.\n', trial, nanCount);
            theta{trial} = theta{trial}(~isnan(theta{trial}));
        end
